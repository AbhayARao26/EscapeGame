...
public class LevelGenerator : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Delete) || Input.GetKeyDown(KeyCode.Backspace))
        {
            ...
            if (hit)
            {
                Point pointToDelete = hit.collider.gameObject.GetComponent<Point>();
                
                if (pointToDelete != null)
                {
                    // Remove point from level
                    _level.Points.RemoveAll(p => (int)p.w == pointToDelete.Id);
                    
                    // Remove associated edges
                    List<Vector2Int> edgesToRemove = _level.Edges
                        .FindAll(e => e.x == pointToDelete.Id || e.y == pointToDelete.Id);
                    
                    foreach (var edge in edgesToRemove)
                    {
                        _level.Edges.Remove(edge);
                        
                        // Remove from edges dictionary
                        edges.Remove(edge);
                        edges.Remove(new Vector2Int(edge.y, edge.x));
                    }
                    
                    // Destroy the point GameObject
                    Destroy(pointToDelete.gameObject);
                    
                    // Remove from points dictionary
                    points.Remove(pointToDelete.Id);
                    
                    // Mark level as dirty for Unity to save changes
                    EditorUtility.SetDirty(_level);
                }
            }
        }
     if (Input.GetKeyDown(KeyCode.X))
    {
        ...
        if (hit)
        {
            ...
            if (hitPoint != null)
            {
                ...
                List<Vector2Int> connectedEdges = _level.Edges
                    .FindAll(e => e.x == hitPoint.Id || e.y == hitPoint.Id);
                
                // Remove edges from level
                foreach (var edge in connectedEdges)
                {
                    // Remove from level edges
                    _level.Edges.Remove(edge);
                    
                    // Destroy edge GameObjects
                    if (edges.TryGetValue(edge, out Edge edgeToRemove))
                    {
                        Destroy(edgeToRemove.gameObject);
                    }
                    
                    // Remove from edges dictionary (both directions)
                    edges.Remove(edge);
                    edges.Remove(new Vector2Int(edge.y, edge.x));
                }
